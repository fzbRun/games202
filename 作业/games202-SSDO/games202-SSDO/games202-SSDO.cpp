...
int main() {
    ...
    MyShader SSDO_shader("shader\\SSDO.vert", "shader\\SSDO.frag");
    ...
    glGenFramebuffers(1, &SSDO_FBO);
    glBindFramebuffer(GL_FRAMEBUFFER, SSDO_FBO);

    unsigned int SSDO_texture;
    glGenTextures(1, &SSDO_texture);
    glBindTexture(GL_TEXTURE_2D, SSDO_texture);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, SCR_WIDTH, SCR_HEIGHT, 0, GL_RGB, GL_FLOAT, NULL);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, SSDO_texture, 0);
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, 4, 4, 0, GL_RGB, GL_FLOAT, &ssdoNoise[0]);
    ...
    */
    ...
    for (GLuint i = 0; i < 64; ++i)
    {
        ...
        scale = 1.0f + 0.1f * (scale * scale - 1.0f);
        sample *= scale;
    ...
    SSDO_shader.use();
    SSDO_shader.setMaxtrix4fv("projection", projection);
    for (int i = 0; i < 64; i++) {
        SSDO_shader.setVec3("samplers[" + std::to_string(i) + "]", ssdoKernel[i]);
    }
    SSDO_shader.setInt("normalAndDepth_texture", 0);
    SSDO_shader.setInt("color_texture", 1);
    SSDO_shader.setInt("noise_texture", 2);

    scene_final_shader.use();
    scene_final_shader.setMaxtrix4fv("projection", projection);
    scene_final_shader.setInt("colorAndOcclusion_texture", 0);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        G_buffer_shader.setVec3("objectColor", vec3(1.0f));
        renderCube();
        model = glm::mat4();
        ...
        model = glm::scale(model, glm::vec3(0.5f));
        G_buffer_shader.setMaxtrix4fv("model", model);
        G_buffer_shader.setVec3("objectColor", vec3(0.0f, 0.0f, 1.0f));
