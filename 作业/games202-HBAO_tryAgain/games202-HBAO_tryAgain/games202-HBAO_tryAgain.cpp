...
void renderPlane(MyShader& shader) {
...
int main() {
    ...
    MyShader G_buffer_shader("shader\\G_buffer.vert", "shader\\G_buffer.frag");
    MyShader HBAO_shader("shader\\HBAO.vert", "shader\\HBAO.frag");
    ...
    unsigned int HBAO_FBO;
    glGenFramebuffers(1, &HBAO_FBO);
    glBindFramebuffer(GL_FRAMEBUFFER, HBAO_FBO);

    unsigned int HBAO_texture;
    glGenTextures(1, &HBAO_texture);
    glBindTexture(GL_TEXTURE_2D, HBAO_texture);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RED, SCR_WIDTH, SCR_HEIGHT, 0, GL_RED, GL_FLOAT, NULL);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, HBAO_texture, 0);
    ...
    G_buffer_shader.use();
    G_buffer_shader.setMaxtrix4fv("projection", projection);
    
    HBAO_shader.use();
    HBAO_shader.setMaxtrix4fv("projection", projection);
    HBAO_shader.setInt("normalAndDepth_texture", 0);
    HBAO_shader.setInt("noise_texture", 1);

    scene_final_shader.use();
    scene_final_shader.setMaxtrix4fv("projection", projection);
    scene_final_shader.setInt("occlusion_texture", 0);
    scene_final_shader.setInt("G_buffer_normalAndDepth_texture", 1);
    scene_final_shader.setInt("G_buffer_color_texture", 2);

    glEnable(GL_DEPTH_TEST);
    glEnable(GL_CULL_FACE);

    /*
    MyShader test_shader("shader\\test.vert", "shader\\test.frag");
    test_shader.use();
    test_shader.setMaxtrix4fv("projection", projection);
    */
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        /*
        test_shader.use();
        test_shader.setMaxtrix4fv("view", view);
        test_shader.setMaxtrix4fv("model", model);
        test_shader.setVec3("lightPos", vec3(view * vec4(lightPos, 1.0f)));
        model = glm::mat4();
        ...
        */
        ...
        G_buffer_shader.use();
        G_buffer_shader.setMaxtrix4fv("view", view);
        model = glm::mat4();
        model = glm::translate(model, glm::vec3(0.0, -2.0f, 0.0f));
        model = glm::scale(model, glm::vec3(20.0f, 1.0f, 20.0f));
        G_buffer_shader.setMaxtrix4fv("model", model);
        renderCube();
        model = glm::mat4();
        model = glm::translate(model, glm::vec3(0.0f, -0.5f, 5.0f));
        model = glm::rotate(model, radians(-90.0f), glm::vec3(1.0, 0.0, 0.0));
        ...
        glClear(GL_COLOR_BUFFER_BIT);
        scene_final_shader.use();
        scene_final_shader.setVec3("lightPos", vec3(view * vec4(lightPos, 1.0f)));
        glActiveTexture(GL_TEXTURE0);
        glBindTexture(GL_TEXTURE_2D, blur2_texture);
        glActiveTexture(GL_TEXTURE1);
        glBindTexture(GL_TEXTURE_2D, G_buffer_normalAndDepth_texture);
